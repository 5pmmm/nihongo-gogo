...
function App() {
  ...
  const renderContent = () => {
    switch (mode) {
      ...
      case AppMode.CHAT: return <motion.div key="chat" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><ChatInterface onBack={() => setMode(AppMode.LEARN)} onXpUpdate={(xp) => addXp({ xp, chat: 1 })} /></motion.div>;
      case AppMode.LISTENING: return <motion.div key="listen" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><ListeningPractice onBack={() => setMode(AppMode.LEARN)} onComplete={() => addXp({ xp: 150, listening: 1 })} /></motion.div>;
  ...
  return (
    <>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex gap-1 sm:gap-2">
            {[
              ...
              { m: AppMode.LISTENING, i: <EarIcon />, l: '聽力' },
      ...
      <main className="max-w-5xl mx-auto py-10 px-6 pb-28 md:pb-20">
      ...
      <motion.nav 
        ...
        className="fixed bottom-0 left-0 right-0 w-full z-50 bg-white/95 backdrop-blur-md border-t border-zinc-200 px-0.5 pt-2 pb-6 xs:pb-7 md:hidden flex justify-around items-center rounded-t-[1.75rem] shadow-[0_-10px_30px_-5px_rgba(0,0,0,0.1)] pb-safe-extra"
      >
        {[
          { m: AppMode.LEARN, i: <BookIcon />, l: '學習' },
          { m: AppMode.LISTENING, i: <EarIcon />, l: '聽力' },
